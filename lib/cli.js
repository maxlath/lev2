...
export async function cli (db, args) {
  ...
  if (hasStreamArg(args)) {
    args.keyEncoding ??= 'utf8'
    args.valueEncoding ??= 'utf8'
    ...
    let Stream
    if (args.keys && !args.values) Stream = KeyStream
    else if (args.values && !args.keys) Stream = ValueStream
    else Stream = EntryStream
